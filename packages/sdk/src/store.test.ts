...
describe("MemoryStore API tests", () => {
  ...
  describe("endTask — selfReport weight updates", () => {
    it("should update memory weight via selfReport", async () => {
      ...
      await drizzleDb.insert(schema.memories).values({
        ...
        embedding: vecBuf(await mockEmbed("Auth middleware is in src/middleware/auth.rs")),
        category: "correction",
        weight: 1.0,
        createdAt: now,
      });
      ...
      await drizzleDb
        ...
        .onConflictDoUpdate({
          target: [schema.memoryRetrievals.memoryId, schema.memoryRetrievals.taskId],
  ...
  describe("decay", () => {
    it("should apply multiplicative decay to all memories", async () => {
      ...
      await drizzleDb.run(sql`UPDATE memories SET weight = weight * ${decayRate}`);
  ...
  describe("purge", () => {
    it("should delete memories below weight threshold", async () => {
      ...
      const purged = await drizzleDb.run(sql`DELETE FROM memories WHERE weight < 0.5`);
  ...
  describe("contradictMemory", () => {
    it("should delete the memory and its retrievals", async () => {
      ...
      await drizzleDb.delete(schema.memories).where(eq(schema.memories.id, memId));
      await drizzleDb
    ...
    it("should optionally create a correction memory", async () => {
      ...
      const correctionMem = await drizzleDb
        .select({ content: schema.memories.content, weight: schema.memories.weight })
        .from(schema.memories)
        .get();
