import type { MemoryCategory, TaskBaseline, UserInputSource } from "./types.js";
...
export function updateBaseline(
	...
	return {
		...
		m2UserCorrections:
			baseline.m2UserCorrections +
			dUserCorr * (userCorrections - meanUserCorrections),
	};
}
...
export function computeTaskScore(
	...
	const zTokens =
		(tokens - baseline.meanTokens) / stddev(baseline.m2Tokens, baseline.count);
	const zErrors =
		(errors - baseline.meanErrors) / stddev(baseline.m2Errors, baseline.count);
	const zUserCorr =
		(userCorrections - baseline.meanUserCorrections) /
		stddev(baseline.m2UserCorrections, baseline.count);
...
export function computeCredit(
	...
	return (
		taskScore *
		(selfReportScore / 3.0) *
		(1.0 / Math.max(numMemoriesRetrieved, 1))
	);
}
...
export function initialWeight(
	...
	switch (category) {
		...
		case "insight":
			return 1.0;
		case "consolidated":
			return 1.0; // set to avg of sources by caller
		default:
			return 1.0;
	}
}
