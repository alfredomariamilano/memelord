...
async function hookStop(): Promise<void> {
	...
	try {
			...
			if (totalTokens >= 50_000) {
				...
				for (const s of sequence) {
					if (
						...
						exploration.reads++;
					else if (["Grep", "Glob", "LSP"].includes(s.tool))
				...
				if (existsSync(sessionFile)) {
					try {
						...
						if (injectedIds.length > 0) {
							...
							if (penalized > 0) {
								console.error(
									`memelord: penalized ${penalized} injected memories (session used ${Math.round(totalTokens / 1000)}k tokens)`,
								);
							}
						}
					} catch {}
		...
		if (existsSync(failuresFile)) {
			const failures = readFileSync(failuresFile, "utf-8")
				.trim()
				.split("\n")
				.map((l) => {
					try {
			...
			for (const f of failures)
				toolFailCounts.set(
					f.tool_name,
					(toolFailCounts.get(f.tool_name) ?? 0) + 1,
				);
