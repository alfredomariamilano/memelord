...
describe("createEmbedder caching", () => {
  ...
  it("should create different embedders for different opts", async () => {
    ...
    const embedder1 = await createEmbedder({ model: "Xenova/all-MiniLM-L6-v2", quantized: true });
    const embedder2 = await createEmbedder({ model: "Xenova/all-MiniLM-L6-v2", quantized: false });
