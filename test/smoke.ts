...
const result1 = await store.startTask(
	"Fix the authentication bug in the login flow",
);
console.log(
	`startTask OK — taskId: ${result1.taskId}, memories: ${result1.memories.length}`,
);
console.assert(
	result1.memories.length === 0,
	"should have no memories initially",
);
...
for (const mem of result2.memories) {
	console.log(
		`  [${mem.category}] w=${mem.weight.toFixed(2)} score=${mem.score.toFixed(3)}: ${mem.content.slice(0, 60)}...`,
	);
}
...
for (const mem of stats.topMemories) {
	console.log(
		`  [w=${mem.weight.toFixed(2)}, used=${mem.retrievalCount}x] ${mem.content.slice(0, 60)}...`,
	);
}
...
console.assert(
	purged >= 1,
	"should have purged at least the low-weight memory",
);
console.assert(
	statsAfter.totalMemories < statsBefore.totalMemories,
	"total memories should decrease after purge",
);

// --- Test 9: Decay ---
const decayResult = await store.decay();
console.log(
	`Decay: ${decayResult.decayed} decayed, ${decayResult.deleted} deleted`,
);
